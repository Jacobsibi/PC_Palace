...
export const getServerSideProps = async context => {
	...
	let defaultBuildSlug = "-high-gaming-custom-pc";
	if (query && query.key) {
		// Extract the value of the 'key' attribute
		const key = query.key;

		// You can also conditionally execute different logic based on the key value
		if (key === 'medium-gaming') {
			// Perform specific actions for the 'low-end-gaming' key value
			// ...
			defaultBuildSlug = "medium-gaming-custom-pc";
		} else {
			// Handle other key values
			// ...
